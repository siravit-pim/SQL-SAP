--Serch Billing, is add-ons
select [@Bill].DocNum 'BillNo',[@Bill1].U_RefDocNum 'DucumentNo'
,case when [@Bill1].U_RefDocType = '13' then 'Inv'
   when [@Bill1].U_RefDocType = '12' then 'DP'
   when [@Bill1].U_RefDocType = '14' then 'CN'
   END 'DocType',orct.DocNum 'IncomingNo'
from [@Bill1] 
left join [@Bill] on [@Bill].DocEntry = [@Bill1].DocEntry
LEFT JOIN (	SELECT DocEntry,ObjType,DocNum FROM OINV where OINV.canceled = 'N' UNION ALL
			SELECT DocEntry,ObjType,DocNum FROM ORIN where ORIN.canceled = 'N' UNION ALL				
			SELECT DocEntry,ObjType,DocNum FROM ODPI where ODPI.canceled = 'N' 
