-- Combination address in JDT1 in SAPHANA, DB
SELECT
    Concat(
        Concat(
            Concat(
...
CASE
                                        WHEN IFNULL($ [JDT1.U_HOMENO.0], '') = '' THEN ''
...
,
CASE
                        WHEN IFNULL($ [JDT1.U_City.0], '') = '' THEN ''
                        ELSE ' ' || $ [JDT1.U_City.0]
                    END
                ),
CASE
                    WHEN IFNULL($ [JDT1.U_County.0], '') = '' THEN ''
                    ELSE ' ' || $ [JDT1.U_County.0]
                END
            ),
CASE
                WHEN IFNULL($ [JDT1.U_State.0], '') = '' THEN ''
                WHEN left(IFNULL($ [JDT1.U_STATE.0], ''), 7) = N'กรุงเทพ' THEN ' ' || N'กรุงเทพ'
                WHEN left(IFNULL($ [JDT1.U_STATE.0], ''), 3) = N'กทม' THEN ' ' || N'กรุงเทพ'
                WHEN left(IFNULL($ [JDT1.U_STATE.0], ''), 3) = N'BKK' THEN ' ' || N'กรุงเทพ'
                ELSE Concat(N' จังหวัด', $ [JDT1.U_State.0])
            END
        ),
CASE
            WHEN IFNULL($ [JDT1.U_ZipCode.0], '') = '' THEN ''
            ELSE ' ' || $ [JDT1.U_ZipCode.0]
        END
    ) ---------------------
FROM
    DUMMY
