-- Billing - paid (or CN) but will see only in Internal Reconciliation

with Z as (
    select 
        bill.DocEntry,oadm.PrintHeadr'HeadName'
    ...
    join (select concat(bill1.U_RefPrefix,U_RefDocNo)'DocNum',bill1.U_RefDocDate,bill1.U_RefDocDueDate,bill1.U_RefDocType,bill1.DocEntry,bill1.U_PayableAmn,bill1.U_Remaining
            ...
            from [@BILL1] bill1
            left join (select 
                            a.DocEntry,a.DocNum,a.ObjType,IIF(isnull(a.U_ShipToAdd,'')<>'',a.U_ShipToAdd,b.StreetS)'Ship',a.DocTotal
                        from oinv a	
