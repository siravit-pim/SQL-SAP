-- Sales Order with multiple `DocNum` in same field , combine values in 1 field
with A as (
    select 'Invoice' TypeDoc
        ,OINV.DocEntry,OINV.DocNum
    ...
    select 'Transfer' TypeDoc
        ,owtr.DocEntry,Owtr.DocNum
        ...
        ,stuff ( 
            ( select ',' + concat(nnm1.BeginStr,cast(wtr21.RefDocNum  as nvarchar(max)) )
                from wtr21
                left join ORDR on ORDR.DocEntry = wtr21.RefDocEntr and wtr21.ObjectType = 67
                left join nnm1 on nnm1.Series = ordr.Series
